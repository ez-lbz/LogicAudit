...
from prompts.report_gen import REPORT_GEN_KNOWLEDGE_BASE
...
5. Output the final audit report as valid JSON

## Output Format
Output ONLY valid JSON (no markdown code blocks, just raw JSON or wrapped in code block):
```json
{
...
def create_report_generator_agent(llm) -> FunctionAgent:
    ...
    return FunctionAgent(
        ...
        system_prompt=f"{REPORT_GEN_KNOWLEDGE_BASE}\n\n{REPORT_GEN_TASK}",
        llm=llm,
        tools=state_tools,
        can_handoff_to=[]
    )
